...
Ex. Here the NGINX Ingress Controller is deployed as a `LoadBalancer` with `NodePort` 32121 and 30884 open for http and https traffic respectively.
...
Enterprise Gateway service. The EG helm chart is configured with an ingress template. A sample template ingress configuration 
...
Ex. Edit etc/kubernetes/helm/values.yaml to the desired configurations to be passed to the ingress template
```bash
ingress:
