...
class PythonKernelBaseKubernetesTestCase(PythonKernelBaseTestCase):
    """
    Python related tests cases common to Spark on Yarn
    """
    ...
    def test_get_resource_manager(self):
        result = self.kernel.execute("sc.getConf().get('spark.master')")
        self.assertRegexpMatches(result, 'k8s.*')
...
class TestPythonKernelKubernetes(unittest.TestCase, PythonKernelBaseKubernetesTestCase):
    KERNELSPEC = os.getenv("PYTHON_KERNEL_KUBERNETES_NAME", "spark_python_kubernetes")
